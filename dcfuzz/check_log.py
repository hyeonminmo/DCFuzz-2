...
logger = logging.getLogger('dcfuzz.check_log')


def is_dcfuzz_log(log_file):
    log = None
    try:
        ...
        return False
    if 'dcfuzz_args' in log:
        return True
    return False


def get_timeout_from_log(log):
    dcfuzz_args = log['dcfuzz_args']
    dcfuzz_config = log['dcfuzz_config']
    timeout = dcfuzz_args['timeout']
...
if __name__ == '__main__':
    ...
    if args.directory:
        ...
        if is_dcfuzz_log(args.log_file):
            ret = check_log_one(log_file=args.log_file, timeout=args.timeout)
        else:
            logger.error('{args.log_file} is not dcfuzz\'s log')
            exit(1)
        if ret:
            exit(0)
        else:
